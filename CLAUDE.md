...
| `ecosystem.vps.config.cjs`, `deploy/`, `VPSCONFIGURATION.md` | Production VPS AlmaLinux 9 (217.160.186.250) https://board.kantoaplo.com, base `boardKantoAplo` : PM2 (127.0.0.1, secrets vérifiés), nginx existant avec 3 autres sous-domaines (ajout additif dans conf.d), Let's Encrypt (`deploy/vps/setup-https.sh`). Ne jamais modifier la conf des autres sites. |
...
npm run admin:create                  # super admin en console (--promote, --reset-password, admin:list)
