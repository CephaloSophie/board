    ...
    <>
      <header className="app-header">
        ...
        {projects && projects.length > 0 && (
          <div className="project-switcher ctrl">
            ...
            >
              ...
              {projectKey && project && !projects.some((p) => p.key === projectKey) && <option value={projectKey}>{project.name}</option>}
              {projects.map((p) => (
        ...
        <div className="header-actions">
          ...
          {user?.role === 'superadmin' && (
            <Link to="/admin/users" className={`btn ghost small${location.pathname.startsWith('/admin') ? ' active-link' : ''}`}>
              Utilisateurs
            </Link>
          )}
      ...
      {project?.archived && (
        <div className="archived-banner">
          ...
          {project.myRole === 'admin' && <Link to={`/projects/${project.key}/settings/danger`}>Désarchiver</Link>}
        </div>
      )}
    </>
  );
